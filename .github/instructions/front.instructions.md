...
爱印云打印平台 前端是一个基于 **Vue 3 + TypeScript + Naive UI** 的现代化管理后台应用，提供设备管理、打印任务、固件升级等核心功能。

## API接口

接口后端的OPENAPI文档地址：https://127.0.0.1:8000/api.json，严格按照该文档定义的接口进行前后端交互。
前端通过 RESTful API 与后端服务通信，所有接口均定义在 `src/api/modules/` 目录下。每个模块对应一个独立的 API 文件，包含接口类型定义和请求方法。

## 注意事项

- **编译测试结果**： 所有新增代码必须通过编译测试，确保无类型错误和编译警告。
...
本文档涵盖了 爱印云打印平台 前端项目的核心开发规范和最佳实践。在创建新模块时，请：
